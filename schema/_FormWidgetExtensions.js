...
dojo.extend(dijit.form.DateTextBox, {
    ...
    _setJsonValueAttr: function(value) {
        ...
        var result = map[this.format] ? this.attr('value', map[this.format](value)) : this.attr('value', map['date-time'](value));
        return result;
    }
});
...
dojo.extend(dijit.form.TimeTextBox, {
    ...
    _setJsonValueAttr: function(value) {
        return this.attr('value', dojo.date.locale.parse(val, {selector: 'time', timePattern: 'hh:mm:ss'}));
    }
});
